...
import { jurnalFromSlug, JURNAL_SOURCE_LABELS } from "@/lib/jurnal-utils";
  ...
  if (!jurnal) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/finance/buku-besar/jurnal-umum" className="text-brand text-[13px] font-semibold mt-2 inline-block">Kembali</Link>
      </div>
    );
  }
    ...
    <div>
      <PageHeader
        ...
        breadcrumb={[
          ...
          { label: "Jurnal Umum", href: "/finance/buku-besar/jurnal-umum" },
          { label: row.id },
        ]}
        actions={<StatusBadge status={row.status} />}
      />

      <Link href="/finance/buku-besar/jurnal-umum" className="inline-flex items-center gap-1 text-[13px] text-brand font-semibold mb-4 hover:underline">
        ...
        <div className="lg:col-span-1 bg-white border border-slds-border rounded-lg p-4 space-y-2 text-[13px]">
          ...
          {row.sourceType && (
            <div className="flex justify-between">
              <span className="text-slds-text-weak">Sumber</span>
              <span className="font-semibold">{JURNAL_SOURCE_LABELS[row.sourceType]}</span>
            </div>
          )}
          {row.refFaktur && (
            <div className="flex justify-between">
              <span className="text-slds-text-weak">Ref. Faktur</span>
              <span className="font-mono font-semibold">{row.refFaktur}</span>
            </div>
          )}
          {row.refAdj && (
            <div className="flex justify-between">
              <span className="text-slds-text-weak">Ref. Penyesuaian</span>
              <Link href={`/finance/persediaan/penyesuaian-persediaan/${row.refAdj.toLowerCase()}`} className="font-mono font-semibold text-brand hover:underline">
                {row.refAdj}
              </Link>
            </div>
          )}
          {row.refPayment && (
            <div className="flex justify-between">
              <span className="text-slds-text-weak">Ref. Bukti</span>
              <span className="font-mono font-semibold">{row.refPayment}</span>
            </div>
          )}
          {row.refOpb && (
            <div className="flex justify-between">
