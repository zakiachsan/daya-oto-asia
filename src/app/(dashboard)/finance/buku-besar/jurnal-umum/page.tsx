...
import { allocateJurnalId, jurnalSlug, JURNAL_SOURCE_LABELS, linesFromDraft, type JurnalDetail } from "@/lib/jurnal-utils";
...
export default function JurnalPage() {
  ...
  function handlePost() {
    ...
    const newJurnal: JurnalDetail = {
      id: allocateJurnalId(items),
      tanggal,
      keterangan: keterangan.trim(),
      debit: totalDebit,
      kredit: totalCredit,
      status: "Posted",
      sourceType: "jurnal-manual",
        ...
        columns={[
          {
            ...
            className: "font-mono",
            render: (r) => (
              <Link href={`/finance/buku-besar/jurnal-umum/${jurnalSlug(String(r.id))}`} className="font-mono font-semibold text-brand hover:underline">
                {String(r.id)}
              </Link>
            ),
          },
          { key: "tanggal", label: "Tanggal" },
          {
            key: "sourceType",
            label: "Sumber",
            render: (r) => (r.sourceType ? JURNAL_SOURCE_LABELS[r.sourceType] : "Manual"),
          },
