...
import { useJurnalList, usePenyesuaianStok } from "@/lib/preview-store";
  ...
  if (!found) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/finance/persediaan/penyesuaian-persediaan" className="text-brand text-[13px] font-semibold mt-2 inline-block">Kembali</Link>
      </div>
    );
  }
    ...
    <div>
      <PageHeader
        ...
        breadcrumb={[
          ...
          { label: "Penyesuaian Persediaan", href: "/finance/persediaan/penyesuaian-persediaan" },
          { label: row.id },
        ]}
        actions={<StatusBadge status={row.status} />}
      />

      <Link href="/finance/persediaan/penyesuaian-persediaan" className="inline-flex items-center gap-1 text-[13px] text-brand font-semibold mb-4 hover:underline">
        ...
        <div className="lg:col-span-1 bg-white border border-slds-border rounded-lg p-4 space-y-2 text-[13px]">
          ...
          {row.jurnalId && (
            <div className="flex justify-between">
              ...
              <Link href={`/finance/buku-besar/jurnal-umum/${jurnalSlug(row.jurnalId)}`} className="font-mono font-semibold text-brand hover:underline">{row.jurnalId}</Link>
            </div>
          )}
