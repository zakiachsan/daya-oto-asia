...
export default function HutangPiutangDetailPage() {
  ...
  if (!found) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/finance/hutang-piutang" className="text-brand text-[13px] font-semibold mt-2 inline-block">Kembali</Link>
      </div>
    );
  }
  ...
  const fakturHref = row.refFaktur
    ? row.tipe === "Piutang"
      ? `/finance/faktur-penjualan/${fakturSlug(row.refFaktur)}`
      : `/finance/faktur-pembelian`
    : null;
    ...
    <div>
      <PageHeader
        ...
        breadcrumb={[
          ...
          { label: "Hutang / Piutang", href: "/finance/hutang-piutang" },
          { label: row.pihak },
        ]}
        actions={<StatusBadge status={row.status} />}
      />

      <Link href="/finance/hutang-piutang" className="inline-flex items-center gap-1 text-[13px] text-brand font-semibold mb-4 hover:underline">
